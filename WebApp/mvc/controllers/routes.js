...
module.exports = function(app, passport, Points, User, db) {
	...
	function getTotalPts(usrEmail, callback) {
		var results = Points.aggregate([ { $match: { email: usrEmail } }, { $group: { _id: '$email', totalPoints: { $sum: { $add: [ '$pointAmt' ] } } } } ]).exec(function(err, res) {
	...
	function findPointLog(usrEmail, resultNum, callback) {
		if (resultNum == -1) {
			var results = Points.aggregate([ { $sort: { _id: -1 } }, { $match: { email: usrEmail } } ]).exec(function(err, res) {
				if (err) console.log('Error' + err);
				callback(res);
			});
		} else {
			var results = Points.aggregate([ { $sort: { _id: -1 } }, { $match: { email: usrEmail } }, { $limit: resultNum } ]).exec(function(err, res) {
				if (err) console.log('Error' + err);
				callback(res);
			});
		}
